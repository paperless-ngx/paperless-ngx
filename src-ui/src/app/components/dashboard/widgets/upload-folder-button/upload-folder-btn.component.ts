...
@Component({
  selector: 'pngx-upload-folder-btn',
  templateUrl: './upload-folder-btn.component.html',
  styleUrls: ['./upload-folder-btn.component.scss'],
})
export class UploadFolderButtonComponent extends ComponentWithPermissions {
  ...
  public onFolderSelected(event: Event) {
    const files = (event.target as HTMLInputElement).files
    if (files) {
      const payload = { folder: '', dossiers: '' }
      const folderMatch = this.router.url.match(/folders\/(\d+)/)
      const dossierMatch = this.router.url.match(/dossiers\/(\d+)/)
      ...
      if (firstFilePath.includes('/')) {
        this.uploadDocumentsService.uploadFolder(files, payload) // Gọi API tải thư mục
      } else {
        this.uploadDocumentsService.uploadFiles(files, payload) // Tải từng file nếu không phải thư mục
      }
