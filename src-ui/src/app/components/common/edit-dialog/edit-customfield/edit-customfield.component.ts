...
export enum EditCustomfieldMode {
  CREATE = 0,
  EDIT = 1,
}

@Directive()
export abstract class EditCustomfieldComponent<
...
> implements OnInit {
  ...
  @Input()
  customfieldMode: EditCustomfieldMode = EditCustomfieldMode.CREATE
  ...
  ngOnInit(): void {
    if (this.object != null && this.customfieldMode !== EditCustomfieldMode.CREATE) {
  ...
  getTitle() {
    switch (this.customfieldMode) {
      case EditCustomfieldMode.CREATE:
        return this.getCreateTitle()
      case EditCustomfieldMode.EDIT:
