...
##### Export {#workflow-action-export}

"Export" actions deliver a copy of the document to an [export target](#export-targets) — an S3
bucket, an SFTP host or a local directory. Export is not a backup: the copy is a portable,
per-document file (plus an optional plain-JSON metadata sidecar) intended for people, retention
policies or other systems, not for re-import into paperless. You can specify:

- The export target to deliver to
- Whether to include the original file, the archive version, or both (at least one is required)
- Whether to write a `.metadata.json` sidecar with the document's title, correspondent, document
  type, tags, storage path, custom fields, dates and checksums
- An optional path, which works exactly like a
  [storage path](advanced_usage.md#file-name-handling): the rendered template is the full path and
  filename of the exported file (the extension is appended automatically), e.g.
  `{{ created_year }}/{{ document_type }}/{{ title }}`. If left empty, exported files are named
  exactly as in the media directory — i.e. following the document's storage path or the global
  filename format, or the document id if neither is set
- What to do when the destination already has an object under the same name:
    - **Overwrite existing** (default): the previous copy is replaced, so re-exporting a changed
      document (e.g. via a "Document Updated" trigger) does not accumulate near-duplicates
    - **Skip if exists**: the existing object is left untouched and nothing is uploaded
    - **Keep both (add suffix)**: the new delivery is written under a version suffix
      (e.g. `invoice.v20240512103000.pdf`); the file, archive version and sidecar of one delivery
      share the same suffix so they stay a matching set

Regardless of the chosen policy, if the destination itself refuses an overwrite — for example an
S3 object under an Object Lock retention — a second, version-suffixed copy is written instead of
failing the export. Overwriting can also be prevented destination-side entirely, e.g. with S3
Object Lock or bucket permissions.

Export is pure push: paperless never deletes anything from a target, and needs no delete
permission on it. Deliveries happen in the background with automatic retries; each delivery is
recorded and the history is visible on the document detail page, where documents can also be
exported manually ("Export now"). A delivery that fails after all retries surfaces in the system
status and as a failed task in the tasks view, linked to the document. Once the cause is fixed —
for example, updated credentials on the target — the failed delivery can be retried from the
document's export history, which re-queues the same export record.

Export actions are not available for "Consumption Started" triggers, because at that point the
document has not been stored yet.
...
Export targets are the destinations that [export actions](#workflow-action-export) deliver
documents to. They are managed on the "Export Targets" page and, like mail accounts, are named,
owned objects with credentials. Three kinds are supported:

- **S3**: bucket, optional key prefix, region, custom endpoint (e.g. DigitalOcean), storage class,
  access key and secret key. Optionally, a number of _retention days_ can be set: each delivered
  object then gets an Object Lock retain-until date that far in the future. This requires Object
  Lock to be enabled on the bucket; the lock _mode_ (`governance` vs. `compliance`) is bucket
  configuration and not set by paperless.
- **SFTP**: host, port, base path, username and password, or a private key with an optional
  passphrase. The server's host key is recorded on the first successful connection and verified
  afterwards; a silently changed host key refuses to connect.
- **Local**: a directory on the server running paperless.

Saving a target runs a connection test — a small probe object is written to the destination, read
back and deleted — and a target that fails the test is not saved. The "Test connection" button in
the dialog runs the same probe on demand. Targets can be paused ("Enabled" off) without deleting
their delivery history.

Exports are currently written unencrypted. SFTP transfers are transport-encrypted; for S3, use
the bucket's server-side encryption and bucket policy as the at-rest protection.

Every delivery is recorded with the object key actually written, the file's SHA-256 checksum and
its status. These records deliberately outlive both the document and the target, so the question
"was this ever exported?" can be answered even after deletion. Upon migration, users and groups
who can add workflows are granted access to export targets, and users who can view documents can
view export history.
