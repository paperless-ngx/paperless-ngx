...
async def search_documents(query: str, ctx: Context, limit: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        return {"total_found": len(results), "query": query, "documents": results}
...
async def get_document_details(document_id: int, ctx: Context) -> dict[str, Any]:
    ...
    try:
        ...
        document = (
            await Document.objects.select_related(
                "correspondent",
                "document_type",
                "storage_path",
            )
        ...
        result = {
            ...
            "storage_path": document.storage_path.path
        ...
        await ctx.error(f"Error retrieving document details: {e}")
        return {"error": str(e)}
...
async def list_documents(
    ...
    try:
        ...
        total_count = await Document.objects.account()
        ...
        await ctx.error(f"Error listing documents: {e}")
        return {"error": str(e), "documents": []}
...
async def get_correspondents(ctx: Context) -> list[dict[str, Any]]:
    ...
    try:
        ...
        await ctx.error(f"Error retrieving correspondents: {e}")
        return []
...
async def get_document_types(ctx: Context) -> list[dict[str, Any]]:
    ...
    try:
        ...
        await ctx.error(f"Error retrieving document types: {e}")
        return []
...
async def get_tags(ctx: Context) -> list[dict[str, Any]]:
    ...
    try:
        ...
        tags = await sync_to_async(list)(Tag.objects.all().order_by("name"))
        ...
        await ctx.error(f"Error retrieving tags: {e}")
        return []
...
async def get_document_statistics(ctx: Context) -> dict[str, Any]:
    ...
    try:
        ...
        total_documents = await Document.objects.account()
        total_correspondents = await Correspondent.objects.account()
        total_document_types = await DocumentType.objects.account()
        total_tags = await Tag.objects.account()

        # Get recent documents count (last 30 days)
        thirty_days_ago = datetime.now() - timedelta(days=30)
        recent_documents = await Document.objects.filter(
            added__gte=thirty_days_ago,
        ).account()
        ...
        await ctx.error(f"Error calculating statistics: {e}")
        return {"error": str(e)}
