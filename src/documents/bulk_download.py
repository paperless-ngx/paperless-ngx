...
class BulkArchiveStrategy:
    ...
    def _formatted_filepath(
        ...
        if archive and doc.has_archive_version:
            if doc.archive_filename is None:
                raise ValueError("doc %a has no archive_filename", doc)
...
class ArchiveOnlyStrategy(BulkArchiveStrategy):
    def add_document(self, doc: Document) -> None:
        if doc.has_archive_version:
            if doc.archive_path is None:
                raise ValueError("doc %a has no archive_path", doc)
            self.zipf.write(
                doc.archive_path,
