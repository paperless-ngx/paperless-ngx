...
def close_connection_pool_on_worker_init(**kwargs):
    ...
    This is necessary because the parent process initializes connection pools
    when parsing the Django configuration before forking.
    Closing these pools prevents child processes from inheriting open connections,
    which is critical to avoid issues with connection pooling after forking.
    """
