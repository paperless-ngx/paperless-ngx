                ...
                try:
                    for event in inotify.read(timeout=timeout_ms):
                        ...
                        if flags.MODIFY in flags.from_mask(event.mask):
                            notified_files.pop(filepath_str, None)
                        else:
                            notified_files[filepath_str] = monotonic()

                    # Check the files against the timeout
                    still_waiting = {}
                    # last_event_time is time of the last inotify event for this file
                    for filepath_str, last_event_time in notified_files.items():
                        ...
                        if waited_long_enough and file_still_exists:
                            ...
                            still_waiting[filepath_str] = last_event_time
