    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            for member in zip_ref.infolist():
                ...
                # Fix Unicode issues with ZIP filenames - respect ZIP encoding standards
                try:
                    # ZIP encoding detection based on the ZIP specification:
                    # 1. Modern ZIP with UTF-8 flag set → UTF-8 (universal)
                    # 2. Old ZIP, no UTF-8 flag → CP437 or local code page
                    # 3. ASCII filenames work regardless of encoding

                    raw_filename = member.filename

                    # First: Check if filename is in NFD format (decomposed Unicode)
                    # NFD means characters like ü are stored as u + combining diaeresis
                    has_combining_marks = any(
                        unicodedata.combining(ch) for ch in raw_filename
                    )

                    # Check for potential NFD corruption by looking for unusual character sequences
                    # that often result from zipfile's incorrect handling of NFD filenames
                    def looks_like_nfd_corruption(filename):
                        """
                        Detect if filename contains patterns that suggest NFD corruption.
                        This is more general than hardcoded patterns.
                        """
                        # Look for sequences that are likely corrupted NFD:
                        # 1. Characters that are rarely used in normal filenames
                        # 2. Character sequences that don't make linguistic sense
                        # 3. Byte patterns that suggest encoding mismatch

                        suspicious_chars = 0
                        for char in filename:
                            code_point = ord(char)
                            # Characters that are suspicious in typical filenames:
                            if (
                                # Combining marks that shouldn't be visible
                                0x0300
                                <= code_point
                                <= 0x036F  # Combining Diacritical Marks
                                or
                                # Latin Extended blocks often involved in encoding issues
                                (
                                    0x00C0 <= code_point <= 0x00FF
                                    and char in "êîôûâÊÎÔÛÂ"
                                )
                                or
                                # Box drawing and similar rarely used in filenames
                                0x2500 <= code_point <= 0x257F  # Box Drawing
                                or
                                # Mathematical symbols rarely in filenames
                                (0x2200 <= code_point <= 0x22FF and char in "╠Ω")
                            ):
                                suspicious_chars += 1

                        # If more than one suspicious character, likely corruption
                        return suspicious_chars > 1

                    has_nfd_corruption = looks_like_nfd_corruption(raw_filename)

                    if has_combining_marks:
                        # File was created on macOS (Finder) or similar - normalize to NFC
                        normalized_filename = unicodedata.normalize("NFC", raw_filename)

                    elif has_nfd_corruption:
                        # This is likely NFD that was incorrectly processed by zipfile
                        # Use a general approach to reconstruct Unicode without hardcoded tables

                        normalized_filename = None

                        # Method 1: Try to reconstruct by accessing raw ZIP data
                        # This bypasses zipfile's automatic "decoding"
                        try:
                            # Convert the corrupted string back to bytes
                            corrupted_bytes = raw_filename.encode(
                                "latin-1",
                                errors="ignore",
                            )
                            ...
                            # Try different encodings to see which produces sensible Unicode
                            for encoding in [
                                "utf-8",
                                "cp1252",
                                "iso-8859-1",
                                ...
                                try:
                                    decoded = corrupted_bytes.decode(
                                        encoding,
                                        errors="strict",
                                    )

                                    # Check if this decoding creates proper Unicode
                                    # Apply NFC normalization to handle any NFD sequences
                                    nfc_normalized = unicodedata.normalize(
                                        "NFC",
                                        decoded,
                                    )

                                    # Score this result based on how reasonable it looks
                                    score = 0

                                    # Basic validation
                                    if not nfc_normalized.isprintable():
                                        continue
                                    if any(c in nfc_normalized for c in '<>:"|?*'):
                                        continue
                                    if any(c in nfc_normalized for c in "□�"):
                                        continue

                                    # Bonus for reasonable length
                                    if 3 <= len(nfc_normalized) <= 200:
                                        score += 5

                                    # Bonus for filename extensions
                                    if nfc_normalized.lower().endswith(
                                        (".pdf", ".doc", ".txt", ".jpg", ".png"),
                                    ):
                                        score += 10

                                    # Bonus for common German/European characters
                                    european_chars = sum(
                                        1
                                        for c in nfc_normalized
                                        if c in "äöüÄÖÜßàáâãåæçèéêëìíîïñòóôõøùúûýÿ"
                                    )
                                    score += european_chars * 15

                                    # Penalty for suspicious sequences still present
                                    if looks_like_nfd_corruption(nfc_normalized):
                                        score -= 20

                                    # Bonus for not containing the original corruption markers
                                    if (
                                        "╠" not in nfc_normalized
                                        and "ê" not in nfc_normalized
                                    ):
                                        score += 10
                                    ...
                                    if score > 0 and (
                                        ...
                                        normalized_filename = nfc_normalized
                                        best_score = score

                                except UnicodeDecodeError:
                                    continue

                            # Additional step: Check for systematic byte corruption patterns
                            # Sometimes ZIP files have systematic byte shifts or replacements
                            if (
                                normalized_filename
                                and any(c in normalized_filename for c in "êüÇ")
                                and any(c in raw_filename for c in "╠")
                            ):
                                # This looks like systematic corruption of accented characters
                                # Pattern analysis: vowel + corruption_marker -> accented_character
                                corrected = normalized_filename

                                # Look for various corruption patterns
                                corrections = []
                                for i, char in enumerate(corrected):
                                    if i > 0:
                                        prev_char = corrected[i - 1]

                                        # Umlaut patterns (vowel + ê -> umlaut)
                                        if char == "ê":
                                            if prev_char.lower() == "a":
                                                corrections.append(
                                                    (
                                                        i - 1,
                                                        "ä"
                                        ...
                                        # Acute accent patterns (vowel + ü -> acute)
                                        elif char == "ü":
                                            if prev_char.lower() == "a":
                                                corrections.append(
                                                    (
                                        ...
                                        # Grave accent patterns (vowel + Ç -> grave)
                                        elif char == "Ç":
                                            if prev_char.lower() == "a":
                                                corrections.append(
                                                    (
                                ...
                                for pos, replacement in reversed(corrections):
                                    corrected = (
                                        corrected[:pos]
                                        + replacement
                                        + corrected[pos + 2 :]
                                ...
                                if corrected != normalized_filename:
                                    normalized_filename = corrected

                        except Exception:
                            pass

                        # Method 2: If reconstruction failed, try decomposition analysis
                        if normalized_filename is None:
                            try:
                                # Apply NFKD decomposition and then recompose
                                decomposed = unicodedata.normalize("NFKD", raw_filename)
                        ...
                        if normalized_filename is None:
                            # Encoding detection will handle this below
                            has_nfd_corruption = False

                    elif all(ord(c) < 128 for c in raw_filename):
                        # Pure ASCII - no encoding issues
                        normalized_filename = raw_filename

                    elif not has_combining_marks and not has_nfd_corruption:
                        # Non-ASCII, but no combining marks - check encoding
                        # Check if UTF-8 flag is set in the ZIP entry
                        # The flag_bits field bit 11 (0x800) indicates UTF-8 encoding
                        utf8_flag_set = bool(member.flag_bits & 0x800)

                        if utf8_flag_set:
                            # Modern ZIP with UTF-8 flag: filename is already properly encoded
                            log.debug(f"UTF-8 flag set for {raw_filename!r}")
                            normalized_filename = raw_filename
                        else:
                            # Old ZIP without UTF-8 flag: need to detect proper encoding
                            log.debug(
                                f"No UTF-8 flag for {raw_filename!r}, detecting encoding",
                            )

                            # Python's zipfile already decoded the filename, but possibly incorrectly
                            # Try to get back to raw bytes and re-decode with proper encoding
                            filename_bytes = raw_filename.encode(
                                "latin-1",
                                errors="ignore",
                            )
                            log.debug(f"Raw bytes: {filename_bytes.hex()}")

                            # Try encodings in order of likelihood for old ZIPs
                            encodings_to_try = [
                                "cp1252",  # Windows Western (best umlaut support)
                                "iso-8859-1",  # Latin-1 (good umlaut support)
                                "cp850",  # DOS Western Europe
                                "cp437",  # ZIP default (limited umlaut support)
                                "utf-8",  # Sometimes works even without flag
                            ]

                            best_result = raw_filename  # fallback
                            best_score = -1

                            for encoding in encodings_to_try:
                                try:
                                    decoded = filename_bytes.decode(
                                        encoding,
                                        errors="strict",
                                    )

                                    # Score this decoding attempt
                                    score = 0

                                    # Must be printable and valid filename
                                    if not decoded.isprintable():
                                        continue
                                    if any(c in decoded for c in '<>:"|?*'):
                                        continue
                                    if any(
                                        c in decoded for c in "□�"
                                    ):  # Clear encoding failures
                                        continue

                                    # Check if this creates reasonable text
                                    # Look for common patterns that indicate correct decoding

                                    # Bonus for German characters (if this is a German document)
                                    german_chars = sum(
                                        1 for c in decoded if c in "äöüÄÖÜß"
                                    )
                                    if german_chars > 0:
                                        score += german_chars * 10

                                    # Bonus for reasonable filename patterns
                                    if decoded.lower().endswith(
                                        (".pdf", ".doc", ".txt", ".jpg", ".png"),
                                    ):
                                        score += 5

                                    # Penalty for suspicious character sequences that suggest wrong encoding
                                    # These patterns often indicate double-encoding or wrong codepage
                                    suspicious_patterns = [
                                        "â€",
                                        "Â",
                                        "Ã¤",
                                        "Ã¶",
                                        "Ã¼",  # UTF-8 interpreted as Latin-1
                                        "Ã¶",
                                        "Ã¤",
                                        "Ã¼",  # Common German double-encoding
                                        "Äö",
                                        "Ä¢",
                                        "Äû",  # Another double-encoding pattern
                                    ]
                                    for pattern in suspicious_patterns:
                                        if pattern in decoded:
                                            score -= 15

                                    # Moderate penalty for unusual characters that might be encoding artifacts
                                    unusual_chars = sum(
                                        1 for c in decoded if ord(c) > 255
                                    )
                                    score -= unusual_chars * 2

                                    log.debug(
                                        f"Encoding {encoding}: {decoded!r} (score: {score})",
                                    )
                                    ...
                                    log.debug(f"Failed to decode with {encoding}")
                                    continue

                            log.debug(
                                f"Best result: {best_result!r} (score: {best_score})",
                            )
                            normalized_filename = best_result

                    # Final normalization: ensure NFC form for consistency and portability
                    # This handles any remaining NFD sequences and ensures canonical form
                    normalized_filename = unicodedata.normalize(
                        "NFC",
                        normalized_filename,
                    )
                    ...
                    # If all encoding detection fails, just normalize what we have
                    log.warning(
                        f"Encoding detection failed for {member.filename!r}: {e}",
                    )
                    normalized_filename = unicodedata.normalize("NFC", member.filename)
