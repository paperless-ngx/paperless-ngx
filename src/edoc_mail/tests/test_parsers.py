...
from edoc_mail.parsers import MailDocumentParser
from edoc_tika.tests.utils import HttpxMockMixin
...
class TestEmailMetadataExtraction(BaseMailParserTestCase):
    ...
    def test_extract_metadata_fail(self):
        ...
        with self.assertLogs("edoc.parsing.mail", level="WARNING") as cm:
            self.assertEqual([], self.parser.extract_metadata("na", "message/rfc822"))
            self.assertIn(
                "WARNING:edoc.parsing.mail:Error while fetching document metadata for na",
                cm.output[0],
            )
...
class TestEmailThumbnailGenerate(BaseMailParserTestCase):
    ...
    @mock.patch("edoc_mail.parsers.MailDocumentParser.generate_pdf")
    @mock.patch("edoc_mail.parsers.make_thumbnail_from_pdf")
