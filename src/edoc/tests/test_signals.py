...
class TestFailedLoginLogging(TestCase):
    ...
    def test_unauthenticated(self):
        ...
        request = HttpRequest()
        request.META = {}
        with self.assertLogs("edoc.auth") as logs:
            handle_failed_login(None, {}, request)
            self.assertEqual(
                logs.output,
                [
                    "INFO:edoc.auth:No authentication provided. Unable to determine IP address.",
                ],
            )
    ...
    def test_none(self):
        ...
        with self.assertLogs("edoc.auth") as logs:
            ...
            self.assertEqual(
                logs.output,
                [
                    "INFO:edoc.auth:Login failed for user `john lennon`. Unable to determine IP address.",
                ],
            )
    ...
    def test_public(self):
        ...
        with self.assertLogs("edoc.auth") as logs:
            ...
            self.assertEqual(
                logs.output,
                [
                    "INFO:edoc.auth:Login failed for user `john lennon` from IP `177.139.233.139`.",
                ],
            )
    ...
    def test_private(self):
        ...
        with self.assertLogs("edoc.auth") as logs:
            ...
            self.assertEqual(
                logs.output,
                [
                    "INFO:edoc.auth:Login failed for user `john lennon` from private IP `10.0.0.1`.",
                ],
            )
