    ...
    if directory:
        if not os.path.isdir(directory):
            ...
            test_file = os.path.join(
                directory,
                f"__Edoc_write_test_{os.getpid()}__",
            )
            try:
...
def paths_check(app_configs, **kwargs):
    ...
    return (
        path_check("EDOC_DATA_DIR", settings.DATA_DIR)
        + path_check("EDOC_TRASH_DIR", settings.TRASH_DIR)
        + path_check("EDOC_MEDIA_ROOT", settings.MEDIA_ROOT)
        + path_check("EDOC_CONSUMPTION_DIR", settings.CONSUMPTION_DIR)
    )
...
def binaries_check(app_configs, **kwargs):
    """
    EDOC requires the existence of a few binaries, so we do some checks
    for those here.
    """

    error = "EDOC can't find {}. Without it, consumption is impossible."
    ...
    if settings.DEBUG:
        return [
            Warning(
                ...
                "also means that edoc will tell anyone various "
        ...
        if settings.OCR_MODE == "skip_noarchive":
            msgs.append(
                Warning(
                    ...
                    "EDOC_OCR_SKIP_ARCHIVE_FILE instead.",
                ),
            )
